...
class TOCEntry:
    def write(self):
        context = {
            ...
            'section_title_text': render_nodes(*self.section.title_nodes),
        }
        
        rendered_self = render_template(toc_entry_template, context)
...
def render_entry_list(entries):
    context = {
        'child_lines': render_nodes(*entries)
    }
    
    return render_template(toc_entry_list_template, context)
